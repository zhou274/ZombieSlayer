	...
	public class ZIRGameController : MonoBehaviour 
	{
		...
		void  LevelUpEffect ()
		{
			...
			if ( messageObject )
			{
				...
				if ( levelUpMessages.Length > 0 )    messageObject.Find("Base/Text").GetComponent<TextMeshProUGUI>().text = levelUpMessages[Mathf.FloorToInt(Random.Range(0, levelUpMessages.Length))];
			}
		...
		IEnumerator GameOver(float delay)
		{
			...
			if ( gameOverCanvas )    
			{
				...
				gameOverCanvas.Find("Base/TextScore").GetComponent<TextMeshProUGUI>().text = "得分 " + score.ToString();
                ...
                gameOverCanvas.Find("Base/TextHighScore").GetComponent<TextMeshProUGUI>().text = "最高分 " + highScore.ToString();
			}
