...
namespace Zigzag
{
	public class ZIRPlayer : MonoBehaviour 
	{
		...
		public static Action Resapawned;
		...
		void  Update ()
		{
            if (gameController.GetComponent<ZIRGameController>().isPause == true && Input.GetMouseButtonDown(0))
            {
                gameController.GetComponent<ZIRGameController>().isPause = false;
                Time.timeScale = 1;
                return;
            }
		...
		IEnumerator SavePosition()
		{
			for(; ; )
			{
				//SaveStraight = RoadStraight;
				SavePos = transform.position;
				SaveRota =transform.rotation;
		...
		{
            gameController.SendMessage("SelectContinue");
